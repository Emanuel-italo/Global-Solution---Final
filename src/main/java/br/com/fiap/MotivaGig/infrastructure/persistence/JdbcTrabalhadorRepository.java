...
public class JdbcTrabalhadorRepository implements TrabalhadorRepository {
    ...
    private Trabalhador mapearTrabalhadorComSenha(ResultSet rs) throws SQLException {
        Trabalhador trabalhador = mapearResultSetParaTrabalhador(rs);
      
        trabalhador.setSenha(rs.getString("senha"));
        return trabalhador;
    }
    ...
    public Trabalhador salvar(Trabalhador trabalhador) {
        ...
        String sql = "INSERT INTO T_TRABALHADOR "
                + "(nome, email, cpf, senha, telefone, tipo_veiculo, pontos, nivel, ativo, created_at, last_update) " // Adicionado 'senha'
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"; 
        ...
        try {
            ...
            stmt.setString(4, trabalhador.getSenha()); 
            
          
            stmt.setString(5, trabalhador.getContato());
            stmt.setString(6, trabalhador.getTipoVeiculo());
            stmt.setInt(7, trabalhador.getPontos());
            stmt.setInt(8, trabalhador.getNivel());
            stmt.setInt(9, 1);

            Timestamp agora = new Timestamp(System.currentTimeMillis());
            stmt.setTimestamp(10, agora);
            stmt.setTimestamp(11, agora);
    ...
    public Trabalhador buscarPorCpfParaLogin(String cpf) throws EntidadeNaoLocalizada {
        
      
        String sql = "SELECT id, nome, email, cpf, senha, telefone, tipo_veiculo, pontos, nivel, ativo "
                + "FROM T_TRABALHADOR WHERE cpf = ? AND ativo = 1";
        String cpfNumerico = cpf != null ? cpf.replaceAll("\\D", "") : ""; 
        ...
        try {
            ...
            if (rs.next()) {
                ...
                return mapearTrabalhadorComSenha(rs); 
            } else {
                throw new EntidadeNaoLocalizada("Trabalhador não encontrado com CPF: " + cpf); 
            }
